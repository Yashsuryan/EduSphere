...
const AddCourse = () => {
  ...
  const handleChapter = (action, chapterId) => {
    if(action === 'add') {
      const tittle = prompt('enter Chapter Name');
      if(tittle) {
        const newChapter = {
          chapterId: uniqid(),
          chapterTitle: tittle,
        ...
        <form onSubmit={handleSubmit} className='flex flex-col gap-4 max-w-md w-full text-gray-500'>
          ...
          <div className='flex items-center justify-between flex-wrap'>
            ...
            <div className='flex md:flex-row flex-col items-center gap-3'>
              ...
              <label htmlFor="ThumbanailImage" className='flex items-center gap-3'>
          ...
          <div>
            {chapters.map((chapters, chaptersIndex)=>(
              <div key={chaptersIndex} className='bg-white border rounded-lg mb-4'>
                <div className='flex justify-between items-center p-4 border-b'>
                  <div className='flex items-center'>
                    <img onClick={() => handleChapter('toggle', chapters.chapterId)} src={assets.dropdown_icon} width={14} alt="" className={`mr-2 cursor-pointer transition-all ${chapters.collapsed && '-rotate-90'}`}/>
                    <span className='font-semibold'>{chaptersIndex + 1} {chapters.chapterTitle}</span>
                  </div>
                  <span className='text-gray-500'>{chapters.chapterContent.length} Lectures</span>
                  <img onClick={() => handleChapter('remove', chapters.chapterId)} src={assets.cross_icon} alt="" className='cursor-pointer'/>
                </div>
                {!chapters.collapsed && (
                  <div className='p-4'>
                    {chapters.chapterContent.map((lecture, lectureIndex) => (
                      <div key={lectureIndex} className='flex justify-between items-center mb-2'>
                        ...
                        <img src={assets.cross_icon} alt="" onClick={() => handleLecture('remove', chapters.chapterId, lectureIndex)} className='cursor-pointer'/>
                      </div>
                    ))}
                    <div className='inline-flex bg-gray-100 p-2 rounded cursor-pointer mt-2' onClick={() => handleLecture('add', chapters.chapterId)}>+ Add Lecture</div>
                  </div>
                )}
              </div>
            ))}
